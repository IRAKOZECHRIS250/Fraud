...
st.set_page_config(
    page_title="Fraud prediction on mobile money transactions ",
    page_icon="💰",
    layout="wide",
...
def detect_advanced_anomalies(transaction_data, transaction_type):
    ...
    newbalance_org = transaction_data['newbalanceOrg']
    ...
    # This would require nameOrig and nameDest, but we can infer from balances
    ...
    # 9. Transaction amount exceeds balance
    if amount > oldbalance_org and transaction_type in ["CASH_OUT", "TRANSFER", "PAYMENT"]:
        anomalies.append("Transaction amount exceeds sender balance")
        risk_factors.append(("Amount exceeds balance", "HIGH"))
...
def main():
    # Header
    st.markdown('<div class="main-header">🏦 Advanced Mobile Money Fraud Detection</div>', unsafe_allow_html=True)
    ...
    if predict_button:
        with st.spinner("Performing advanced fraud analysis..."):
            ...
            if prediction is not None:
                # Display results
                st.markdown("---")
                st.markdown('<div class="sub-header">🎯 Advanced Fraud Analysis Results</div>', unsafe_allow_html=True)
                
                # Determine fraud status based on threshold and risk factors
                ml_fraud = probability >= fraud_threshold
                rule_based_fraud = len(anomalies) > 0 and risk_score > 